...
// in simulator we redefine to format for Clima Developer console
...
#define DEFINE_EXCEPTIONS                                                                                      \
  ...
  {                                                                                                            \
    NSString *exceptionName = [@"org.clima." stringByAppendingString:NSStringFromClass([self class])];  \
    TiExceptionThrowWithNameAndReason(exceptionName, reason, subreason, location);                             \
  }                                                                                                            \
                                                                                                               \
  +(void)throwException : (NSString *)reason subreason : (NSString *)subreason location : (NSString *)location \
  {                                                                                                            \
    NSString *exceptionName = @"org.clima";                                                             \
    TiExceptionThrowWithNameAndReason(exceptionName, reason, subreason, location);                             \
  }
...
enum {
  UIClimaNativeItemNone = -1,
  UIClimaNativeItemSpinner = -2,
  UIClimaNativeItemProgressBar = -3,

  UIClimaNativeItemSlider = -4,
  UIClimaNativeItemSwitch = -5,
  UIClimaNativeItemMultiButton = -6,
  UIClimaNativeItemSegmented = -7,

  UIClimaNativeItemTextView = -8,
  UIClimaNativeItemTextField = -9,
  UIClimaNativeItemSearchBar = -10,

  UIClimaNativeItemPicker = -11,
  UIClimaNativeItemDatePicker = -12,

  UIClimaNativeItemInfoLight = -13,
  UIClimaNativeItemInfoDark = -14,

  UIClimaNativeItemDisclosure = -15,

  UIClimaNativeItemContactAdd = -16
};
...
#endif
/**
 *	TiThreadPerformOnMainThread should replace all Clima instances of
