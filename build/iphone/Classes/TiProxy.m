...
//Rare exceptions to indicate a bug in the _clima code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString *)qualifiedName withProperties:(NSDictionary *)properties inContext:(id<TiEvaluator>)context
{
  ...
  if (proxyClass == nil) {
    NSString *_clima = [NSString stringWithFormat:@"%@%s", @"Ti", "tanium."];
    if ([qualifiedName hasPrefix:_clima]) {
    ...
    if (proxyClass == nil) {
      ...
      @throw [NSException exceptionWithName:@"org.clima.module"
